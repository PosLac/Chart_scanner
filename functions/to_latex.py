...
def prepare_data_for_generation(update, orientation, grouped, ratios, bars_with_texts=None, legend_position=None,
                                minMax_array=None,
                                title=None, title_pos=None):
    ...
    plot_options = NoEscape(orientation + ', name=mygraph')
    legend_arguments = ""
    define_color_arguments = []
    coordinates_with_options = []
    ...
    if minMax_array:
        for minMax in minMax_array:
            if minMax[2]:
                plot_options = plot_options + NoEscape(minMax[0] + "=" + str(minMax[1]) + ", ")
    ...
    if grouped:
        ...
        legend_top_right_calculated = (round(legend_top_right_orig.x() / img_width, 2), 1 - round(legend_top_right_orig.y() / img_height, 2))
        print(f"legend_top_right_calculated {legend_top_right_calculated}")

        plot_options += NoEscape(", legend style={at={" + str(legend_top_right_calculated) + "}}")
        print(f"plot_options: {plot_options}")
    ...
    if grouped:
        ...
        for bar_index in range(len(ratios)):
            if orientation == "xbar":
                coordinates.append((ratios[bar_index], bar_index + 1))
            else:
                coordinates.append((bar_index + 1, ratios[bar_index]))
        coordinates_with_options.append((coordinates, f'fill=color{1}'))
    print(f"group_coordinates: {coordinates_with_options}")

    generate_latex(plot_options, legend_arguments, define_color_arguments, coordinates_with_options)


def generate_latex(plot_options, legend_arguments, define_color_arguments, coordinates_with_options):
